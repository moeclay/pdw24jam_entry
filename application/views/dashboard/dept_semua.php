...
<head>
    ...
    <script type="text/javascript">
        ...
        async function getData4(){
            let result1 = await fetch("http://entry.org/admin/laporan/kalkulasidivisi");
            let result  = await result1.json();
            var panjang = result.divisi.length.toString();
            
            var total = result.total_berjalan.sudah_diambil;
            
            let datadivisi = "";
            for(var i=0; i<panjang; i++){
                totaldivisi = result.divisi[i].total_divisi;
                ontime = parseInt(result.divisi[i].total_putih)/parseInt(totaldivisi)*100;
                hijau = parseInt(result.divisi[i].total_hijau)/parseInt(totaldivisi)*100;
                kuning = parseInt(result.divisi[i].total_kuning)/parseInt(totaldivisi)*100;
                merah = parseInt(result.divisi[i].total_merah)/parseInt(totaldivisi)*100;
                ungu = parseInt(result.divisi[i].total_ungu)/parseInt(totaldivisi)*100;
                
                datadivisi += "<tr><td>"+result.divisi[i].nama_divisi.toUpperCase()+"</td><td class='text-center'>"+ontime.toFixed(1)+"% </td><td class='text-center'>"+hijau.toFixed(1)+" %</td><td class='text-center'>"+kuning.toFixed(1)+"%</td><td class='text-center'>"+merah.toFixed(1)+"%</td><td class='text-center'>"+ungu.toFixed(1)+"%</td></tr>";
            }
            document.getElementById("datadivisi").innerHTML = datadivisi;
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview active">
          ...
          <ul class="treeview-menu">
            <li class="active"><a href="<?= site_url('admin/laporan2/dept_semua'); ?>"><i class="fa fa-table" aria-hidden="true"></i> - SEMUA - </a></li>
            ...
            <div class="col-md-12">
                <div class="box box-success">
                    <div class="box-header">
                                ...
                                <p class='text-center'><b>Deparment | Kinerja</b></p>
                            </div>
                    <div class="box-body">
                                <div class="chart-container">
                                    <table class="table table-striped table-responsive" id="divisi">
                                        <thead>
                                            <tr>
                                                <th>Dept/Divisi</th>
